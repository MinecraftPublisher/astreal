...
ptr betterrealloc(ptr original, u4 size) {
    ...
    printf("%p\n", original);
    for(int i = 0; i < size; i++) result[i] = ((byte*)original)[i];
    free(original);
    printf("meow :(\n");
...
ptr betterarray(u4 unit, u4 _count) {
    u4 count = _count + 1;
    u4 *pointer = bettermalloc(unit * count + 2 * sizeof(u4));
    pointer[ -1 ] = 247;
    pointer[ 0 ] = unit;
    pointer[ 1 ] = count;
...
ptr grow(ptr _array, u4 inflation) {
    ...
    u4 *arr2 = betterrealloc(&array[ -2 ], new_size);
...
ptr betterread(int fd) {
    byte mode = 254;
    while (read(fd, &mode, sizeof(byte)) <= 0) {
        printf("%i\n", mode);
    ...
    u4 size;
    read(fd, &size, sizeof(u4));

    ptr data = bettermalloc(sizeof(byte) * size);
...
void betterarraysend(pipeout fd, ptr _array) {
    ...
    for (u8 i = 0; i < (amount * unit); i++) {
        ...
        write(fd.write, &send_array[ i ], sizeof(byte));
    }
}

ptr betterarrayread(pipeout fd) {
    printf("%p\n", &fd);
    u4 unit = ({ u4 *__Data = betterread(fd.read); u4 _Data = *__Data; free(&((byte *) __Data)[ -1 ]); _Data; });
    u4 amount = ({ u4 *__Data = betterread(fd.read); u4 _Data = *__Data; free(&((byte *) __Data)[ -1 ]); _Data; });
    u8 size = unit * amount;
...
int main() {
    __auto_type x = (char *) betterarray(sizeof(char), 5);
